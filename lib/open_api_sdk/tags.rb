...
module OpenApiSDK
  ...
  class Tags
    ...
    def create(request, timeout_ms = nil)
      ...
      if r.status == 201
        if Utils.match_content_type(content_type, 'application/json')
          out = Crystalline.unmarshal_json(JSON.parse(r.env.response_body), ::OpenApiSDK::Shared::TagSchema)
          res.tag_schema = out
        end
      elsif r.status == 400
        if Utils.match_content_type(content_type, 'application/json')
    ...
    def list(request, timeout_ms = nil)
      ...
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Crystalline.unmarshal_json(JSON.parse(r.env.response_body), T::Array[::OpenApiSDK::Shared::TagSchema])
          res.tag_schemas = out
        end
      elsif r.status == 400
        if Utils.match_content_type(content_type, 'application/json')
    ...
    def delete(request, timeout_ms = nil)
      ...
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Crystalline.unmarshal_json(JSON.parse(r.env.response_body), ::OpenApiSDK::Operations::DeleteTagResponseBody)
          res.object = out
        end
      elsif r.status == 400
        if Utils.match_content_type(content_type, 'application/json')
